...
## Residual  Block (RB)
class RB(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.ReLU(inplace=inplace), res_scale=1, dilation=2):
...
class LSRAG(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, act, res_scale, n_resblocks):
        ...
        self.rcab= nn.ModuleList([RB(conv, n_feat, kernel_size, reduction, \
                                       bias=True, bn=False, act=nn.ReLU(inplace=inplace), res_scale=1) for _ in range(n_resblocks)])
        ...
        #     modules_body.append(RCAB(conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.ReLU(inplace=inplace), res_scale=1))
...
class SAN(nn.Module):
    ...
    def __init__(self, conv=common.default_conv, scale=2):
        ...
        # Commented because useless for us (BVOC)
        # RGB mean for DIV2K
